  ...
  if (recPipelineId) {
    useEffect(() => {
      const interval = setInterval(() => {
        fetch('https://anarhist.pythonanyware.com/get_movie_id1', {
          method: 'POST',
    ...
    useEffect(() => {
      const interval = setInterval(() => {
        fetch('https://anarhist.pythonanyware.com/get_generated_caption_url', {
          method: 'POST',
          body: JSON.stringify(recPipelineId),
          headers: { 'content-type': 'application/json' },
        }).then((res) => res.json().then((data1) => {
    ...
    useEffect(() => {
      const interval = setInterval(() => {
        fetch('https://anarhist.pythonanyware.com/get_generated_text', {
          method: 'POST',
          body: JSON.stringify(recPipelineId),
          headers: { 'content-type': 'application/json' },
        }).then((res) => res.json().then((data1) => {
    ...
    useEffect(() => {
      const interval = setInterval(() => {
        fetch('https://anarhist.pythonanyware.com/get_generated_triplets', {
          method: 'POST',
          body: JSON.stringify(recPipelineId),
          headers: { 'content-type': 'application/json' },
        }).then((res) => res.json().then((data1) => {
